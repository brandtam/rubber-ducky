...
describe("addToFrontmatterArray", () => {
  it("creates the field as a single-element array when it doesn't exist", () => {
    const result = addToFrontmatterArray(ARRAY_FIXTURE_BASE, "data_sources", "mercury");
    const parsed = parseFrontmatter(result);
    expect(parsed!.data.data_sources).toEqual(["mercury"]);
  });

  it("appends to an existing array", () => {
    const result = addToFrontmatterArray(ARRAY_FIXTURE_WITH_LIST, "data_sources", "stripe");
    const parsed = parseFrontmatter(result);
    expect(parsed!.data.data_sources).toEqual(["mercury", "plaid", "stripe"]);
  });

  it("is a no-op when the value is already present (unique by default)", () => {
    const result = addToFrontmatterArray(ARRAY_FIXTURE_WITH_LIST, "data_sources", "mercury");
    expect(result).toBe(ARRAY_FIXTURE_WITH_LIST);
  });

  it("duplicates the value when allowDuplicates: true", () => {
    const result = addToFrontmatterArray(ARRAY_FIXTURE_WITH_LIST, "data_sources", "mercury", {
      allowDuplicates: true,
    });
    const parsed = parseFrontmatter(result);
    expect(parsed!.data.data_sources).toEqual(["mercury", "plaid", "mercury"]);
  });

  it("preserves other frontmatter fields", () => {
    const result = addToFrontmatterArray(ARRAY_FIXTURE_BASE, "data_sources", "mercury");
    const parsed = parseFrontmatter(result);
    expect(parsed!.data.title).toBe("Project Page");
    expect(parsed!.data.type).toBe("project");
    expect(parsed!.data.created).toBe("2026-05-09");
  });
  ...
  it("throws FrontmatterArrayTypeError when the field is a scalar", () => {
    const fixture = `---
title: Test
status: backlog
---
...
Body.
`;
    expect(() => addToFrontmatterArray(fixture, "status", "anything")).toThrow(
      FrontmatterArrayTypeError
    );
  });

  it("throws when content has no frontmatter at all", () => {
    expect(() => addToFrontmatterArray("# Just a heading\n", "data_sources", "x")).toThrow(
      /no frontmatter found/
    );
  });

  it("round-trips values with special YAML characters", () => {
    const result = addToFrontmatterArray(
      ARRAY_FIXTURE_BASE,
      "data_sources",
      "sniff:https://www.redfin.com"
    );
    const parsed = parseFrontmatter(result);
    expect(parsed!.data.data_sources).toEqual(["sniff:https://www.redfin.com"]);
  });
});

describe("removeFromFrontmatterArray", () => {
  it("removes the value", () => {
    const result = removeFromFrontmatterArray(ARRAY_FIXTURE_WITH_LIST, "data_sources", "mercury");
    const parsed = parseFrontmatter(result);
    expect(parsed!.data.data_sources).toEqual(["plaid"]);
  });

  it("is a no-op when the value isn't present", () => {
    const result = removeFromFrontmatterArray(ARRAY_FIXTURE_WITH_LIST, "data_sources", "notpresent");
    expect(result).toBe(ARRAY_FIXTURE_WITH_LIST);
  });

  it("leaves the field as [] when the last element is removed", () => {
    const oneElement = `---
title: Test
type: project
created: "2026-05-09"
...
---
`;
    const result = removeFromFrontmatterArray(oneElement, "data_sources", "mercury");
    const parsed = parseFrontmatter(result);
    expect(parsed!.data.data_sources).toEqual([]);
  });

  it("treats missing field as empty array (no-op)", () => {
    const result = removeFromFrontmatterArray(ARRAY_FIXTURE_BASE, "data_sources", "mercury");
    expect(result).toBe(ARRAY_FIXTURE_BASE);
  });
...
---
`;
    expect(() => removeFromFrontmatterArray(fixture, "status", "anything")).toThrow(
      FrontmatterArrayTypeError
    );
  });
});
...
describe("setFrontmatterArray", () => {
  it("replaces the array with the given values", () => {
    const result = setFrontmatterArray(ARRAY_FIXTURE_WITH_LIST, "data_sources", ["stripe", "hubspot"]);
    const parsed = parseFrontmatter(result);
    expect(parsed!.data.data_sources).toEqual(["stripe", "hubspot"]);
  });

  it("sets the field to [] when given an empty array", () => {
    const result = setFrontmatterArray(ARRAY_FIXTURE_WITH_LIST, "data_sources", []);
    const parsed = parseFrontmatter(result);
    expect(parsed!.data.data_sources).toEqual([]);
  });

  it("creates the field when it doesn't exist", () => {
    const result = setFrontmatterArray(ARRAY_FIXTURE_BASE, "data_sources", ["mercury"]);
    const parsed = parseFrontmatter(result);
    expect(parsed!.data.data_sources).toEqual(["mercury"]);
  });

  it("does not require the field to already be an array (overwrite is explicit)", () => {
    const fixture = `---
title: Test
status: backlog
---
`;
    const result = setFrontmatterArray(fixture, "status", ["a", "b"]);
    const parsed = parseFrontmatter(result);
    expect(parsed!.data.status).toEqual(["a", "b"]);
  });
});
