...
describe("asap add CLI", () => {
  ...
  it("fails outside a workspace", () => {
    ...
    try {
      const { stdout, status } = runCliFail(["--json", "asap", "add", "Test"], outsideDir);
...
describe("asap resolve CLI", () => {
  ...
  it("fails with invalid index", () => {
    ...
    const { stdout, status } = runCliFail(["--json", "asap", "resolve", "5"], tmpDir);
    expect(status).not.toBe(0);
    const output = JSON.parse(stdout || "{}");
    expect(output.success).toBe(false);
...
describe("screenshot ingest CLI", () => {
  ...
  it("fails outside a workspace", () => {
    ...
    try {
      const { stdout, status } = runCliFail(
        ["--json", "screenshot", "ingest", imgPath, "Test"],
        outsideDir
      );
      expect(status).not.toBe(0);
      const output = JSON.parse(stdout || "{}");
