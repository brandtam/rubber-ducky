  ...
  const doctor = program
    ...
    .action(async (_opts: unknown, cmd: Command) => {
      ...
      try {
        ...
        if (!result.healthy) {
          // Health check failures are an unclassified state — the user should
          // read the per-check messages to understand what's wrong.
          process.exit(ExitCode.Unclassified);
        }
      } catch (error) {
