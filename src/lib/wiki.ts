...
/**
 * Public JSON contract for `rubber-ducky wiki search`. `matches` carries the
 * results capped to `limit` (or every match under `--verbose`). The caller
 * always sees `totalMatches` (full count), `returnedMatches` (length of
 * `matches`), and `truncated` (whether the cap dropped any).
 */
export interface WikiSearchJson {
  success: true;
  query: string;
  totalMatches: number;
...
/**
 * Shape a `SearchResult` for JSON output. The lib applies the cap so the
 * shape function is the single source of truth for "what does the agent
 * see?" — the CLI just parses the `--limit` flag and forwards it.
 *
 * `--verbose` overrides `limit`: the user wants every match.
 */
export function searchResultToJson(
  result: SearchResult,
  opts: { verbose?: boolean; limit: number },
): WikiSearchJson {
  const matches = opts.verbose ? result.matches : result.matches.slice(0, opts.limit);
  return {
    success: true,
    query: result.query,
    totalMatches: result.totalMatches,
    returnedMatches: matches.length,
    truncated: matches.length < result.totalMatches,
    matches,
  };
}
